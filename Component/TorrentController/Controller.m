...
//        [[(AppDelegate *)[[UIApplication sharedApplication] delegate] torrentController] updateTorrentHistory];
//    }
//    return;
//}
...
- (id)torrentFromHash:(NSString *)hashString
{
    if (hashString == nil)
        return nil;
    for (Torrent *torrent in fTorrents)
        if([[torrent hashString] isEqualToString:hashString])
            return torrent;
...
//        [[[UIAlertView alloc] initWithTitle:@"Add from URL" message:[NSString stringWithFormat:@"Adding from %@ failed. %@", url, [error localizedDescription]]  delegate:nil cancelButtonTitle:@"Dismiss" otherButtonTitles:nil] show];
        NSLog(@"Adding from %@ failed. %@\nhash=%@", url, [error localizedDescription], [[error userInfo] valueForKey:@"hashString"]);
//        Torrent *torrent = [self torrentFromHash:[[error userInfo] valueForKey:@"hashString"]];
//        [self removeTorrents:@[torrent] trashData:true];
    }
...
//    [alertView show];
    NSLog(@"Failed to fetch torrent URL: \"%@\". \nError: %@", url, [error localizedDescription]);
    [fActivities removeObject:fetcher];
    [self decreaseActivityCounter];
}

- (void)removeTorrentsWithHashs:(NSArray*)hashStrings trashData:(BOOL)trashData
{
    NSMutableArray *array = [[NSMutableArray alloc] init];
    for (NSString *str in hashStrings)
        [array addObject:[self torrentFromHash:str]];
    [self removeTorrents:array trashData:trashData];
}
...
- (NSError*)openFile:(NSString*)file addType:(AddType)type forcePath:(NSString *)path
{
    ...
    if (result != TR_PARSE_OK)
    {
        if (result == TR_PARSE_DUPLICATE) {
            error = [[NSError alloc]
                     initWithDomain:@"Controller" code:1
                     userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"Torrent %s already exists. ", info.name],
                                @"hashString":[NSString stringWithUTF8String:info.hashString],
                                @"filePath":file,
                                @"addType":[NSNumber numberWithInt:(int)type]}];
        }
        else if (result == TR_PARSE_ERR)
        {
            error = [[NSError alloc] initWithDomain:@"Controller" code:2 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid torrent file. "] forKey:NSLocalizedDescriptionKey]];
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:NotificationAddNewTorrentFailed object:self userInfo:@{@"error":error}];
    ...
    Torrent * torrent;
    if (!(torrent = [[Torrent alloc] initWithPath:file location: [path stringByExpandingTildeInPath] deleteTorrentFile: YES lib: fLib])) {
        error = [[NSError alloc] initWithDomain:@"Controller" code:3 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Unknown error. "] forKey:NSLocalizedDescriptionKey]];
        [[NSNotificationCenter defaultCenter] postNotificationName:NotificationAddNewTorrentFailed object:self userInfo:@{@"error":error}];
        return error;
    }
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:NotificationNewTorrentAdded object:self userInfo:@{@"torrent":torrent}];
    [self updateTorrentHistory];
    return nil;
}
